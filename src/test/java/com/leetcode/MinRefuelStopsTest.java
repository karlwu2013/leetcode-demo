package com.leetcode;
import org.junit.Assert;
import org.junit.Test;
import org.junit.runner.RunWith;

import java.io.*;
import java.util.Date;
public class MinRefuelStopsTest {
    @Test
    public void test_minRefuelStops(){
        MinRefuelStops st=new MinRefuelStops();
        int target=100;
        int startFuel=10;
        int stations[][]={{10,60},{20,30},{30,30},{60,40}};
        int expect=2;
        int r=st.minRefuelStops(target,startFuel,stations);
        Assert.assertEquals(expect,r);

        target=100;
        startFuel=1;
        int stations2[][]={{10,100}};
        expect=-1;
        r=st.minRefuelStops(target,startFuel,stations2);
        Assert.assertEquals(expect,r);

        target=1;
        startFuel=1;
        int stations3[][]={};
        expect=0;
        r=st.minRefuelStops(target,startFuel,stations3);
        Assert.assertEquals(expect,r);
    }
    @Test
    public void test_minRefuelStops_2(){
        MinRefuelStops st=new MinRefuelStops();
        int target=100;
        int startFuel=50;
        int stations[][]={{25,50},{50,25}};
        int expect=1;
        int r=st.minRefuelStops(target,startFuel,stations);
        Assert.assertEquals(expect,r);
    }
    @Test
    public  void test_minRefuelStops_3(){
        MinRefuelStops st=new MinRefuelStops();
        int target=100;
        int startFuel=50;
        int stations[][]={{25,30}};
        int expect=-1;
        int r=st.minRefuelStops(target,startFuel,stations);
        Assert.assertEquals(expect,r);
    }
    @Test
    public void test_minRefuelStops_4(){
        MinRefuelStops st=new MinRefuelStops();
        int target=1000;
        int startFuel=83;
        int stations[][]=
                        {{15,457},{156,194},{160,156},{230,314},{390,159},{621,20},{642,123},{679,301},{739,229},{751,174}};
        int expect=3;
        int r=st.minRefuelStops(target,startFuel,stations);
        Assert.assertEquals(expect,r);
    }
    @Test
    public void test_minRefuelStops_5(){
        MinRefuelStops st=new MinRefuelStops();
        int target=1000;
        int startFuel=36;
        int stations[][]=
                {{7,13},{10,11},{12,31},{22,14},{32,26},{38,16},{50,8},{54,13},{75,4},{85,2},{88,35},{90,9},{96,35},{103,16},{115,33},{121,6},{123,1},{138,2},{139,34},{145,30},{149,14},{160,21},{167,14},{188,7},{196,27},{248,4},{256,35},{262,16},{264,12},{283,23},{297,15},{307,25},{311,35},{316,6},{345,30},{348,2},{354,21},{360,10},{362,28},{363,29},{367,7},{370,13},{402,6},{410,32},{447,20},{453,13},{454,27},{468,1},{470,8},{471,11},{474,34},{486,13},{490,16},{495,10},{527,9},{533,14},{553,36},{554,23},{605,5},{630,17},{635,30},{640,31},{646,9},{647,12},{659,5},{664,34},{667,35},{676,6},{690,19},{709,10},{721,28},{734,2},{742,6},{772,22},{777,32},{778,36},{794,7},{812,24},{813,33},{815,14},{816,21},{824,17},{826,3},{838,14},{840,8},{853,29},{863,18},{867,1},{881,27},{886,27},{894,26},{917,3},{953,6},{956,3},{957,28},{962,33},{967,35},{972,34},{984,8},{987,12}};
        int expect=32;
        int r=st.minRefuelStops(target,startFuel,stations);
        Assert.assertEquals(expect,r);
    }
}
